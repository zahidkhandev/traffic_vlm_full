...
class QwenVLMValidator:
    ...
    def _run_inference(self, messages):
        ...
        for cls in self.object_classes:
            for variant in [cls, cls.replace(" ", ""), cls.replace(" ", "-")]:
    ...
    def draw_highlighted_box(self, img: Image.Image, box: List[int]) -> Image.Image:
        img_copy = img.copy()
    ...
    def classify_with_context(
        ...
        img_with_highlight = self.draw_highlighted_box(full_img, box)

        w, h = img_with_highlight.size
        max_size = 768
        if w > max_size or h > max_size:
            scale = max_size / max(w, h)
            img_with_highlight = img_with_highlight.resize(
        ...
        messages = [
            {
                "role": "user",
                "content": [
                    {"type": "image", "image": img_with_highlight},
                    {
                        "type": "text",
                        "text": f"Look at the object inside the RED bounding box. What is it? Choose exactly one: {class_list}",
                    },
                ],
            }
        ]
    ...
    def classify_with_coordinates(
        self, full_img: Image.Image, box: List[int]
    ) -> Tuple[Dict[str, float], NDArray[np.float32]]:
        x1, y1, x2, y2 = box
        img_to_use = full_img.copy()
        orig_w, orig_h = img_to_use.size

        max_size = 768
        scale = 1.0
        if orig_w > max_size or orig_h > max_size:
            scale = max_size / max(orig_w, orig_h)
            img_to_use = img_to_use.resize(
        ...
        messages = [
            {
                "role": "user",
                "content": [
                    {"type": "image", "image": img_to_use},
                    {
                        "type": "text",
                        "text": (
                            f"There is an object at pixel coordinates "
                            f"[x={sx1}, y={sy1}, width={sw}, height={sh}] "
                            f"(origin is top-left corner). "
                            f"What is the object at that location? "
                            f"Choose exactly one: {class_list}"
                        ),
                    },
                ],
            }
    ...
    def classify_with_grounding_tokens(
        self, full_img: Image.Image, box: List[int]
    ) -> Tuple[Dict[str, float], NDArray[np.float32]]:
        x1, y1, x2, y2 = box
        img_to_use = full_img.copy()
        ...
        nx1 = int(x1 / orig_w * 1000)
        ny1 = int(y1 / orig_h * 1000)
        nx2 = int(x2 / orig_w * 1000)
        ny2 = int(y2 / orig_h * 1000)
        ...
        messages = [
            {
                ...
                "content": [
                    {"type": "image", "image": img_to_use},
                    {
                        "type": "text",
                        "text": (
                            f"What is the object at "
                            f"<|object_ref_start|>the bounding box<|object_ref_end|>"
                            f"<|box_start|>({nx1},{ny1}),({nx2},{ny2})<|box_end|>? "
                            f"Choose exactly one: {class_list}"
                        ),
                    },
                ],
...
class BDD100KConfidentLearning:
    def __init__(
        ...
        mode_map = {
            "highlight": self.vlm.classify_with_context,
            "coordinates": self.vlm.classify_with_coordinates,
            "grounding_tokens": self.vlm.classify_with_grounding_tokens,
        }
        if inference_mode not in mode_map:
            raise ValueError(f"inference_mode must be one of {list(mode_map.keys())}")
        self.classify_fn = mode_map[inference_mode]
    ...
    def process_dataset(
        ...
        for label_file in pbar_images:
            ...
            for obj_data in pbar_objects:
                ...
                if total_objects == 0:
                    pbar_objects.write(
                        f"\n[First object: {category} at [{x1},{y1},{x2},{y2}]]"
                    )
                    start = time.time()

                pred_probs, embedding = self.classify_fn(full_img, [x1, y1, x2, y2])
    ...
    def compute_confident_joint(self):
        ...
        print("\nComputed confident joint matrix")
    ...
    def save_error_crops(self, errors: List[Dict[str, Any]], run_dir: str):
        ...
        for idx, error in enumerate(tqdm(errors, desc="Saving error crops")):
            ...
            if sample_idx in self.crops_cache:
                ...
                crop.save(os.path.join(error_crops_dir, filename), quality=95)
    ...
    def visualize_embedding_umap(self, output_dir: str):
        ...
        if len(self.embeddings) < 5:
            print("Skipping UMAP, need more samples")
            return
    ...
    def visualize_confident_joint(self, output_dir: str):
        ...
        axes[0].set_title("Confident Joint", fontsize=14, fontweight="bold")
        ...
        axes[1].set_title("Latent Joint", fontsize=14, fontweight="bold")
    ...
    def visualize_noise_matrix(self, output_dir: str):
        ...
        axes[0].set_title("Noise Matrix", fontsize=14, fontweight="bold")
        ...
        axes[1].set_title("Noise Matrix Transposed", fontsize=14, fontweight="bold")
...
if __name__ == "__main__":
    print("BDD100K Confident Learning with Full Image Context")
